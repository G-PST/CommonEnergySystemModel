...
def transform_solve_pattern(timeline: pd.DataFrame,
                            warnings: TransformationWarnings) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Create default solve_pattern entity with start_time_durations map.

    Returns:
        Tuple of (base DataFrame, start_time_durations map DataFrame)
    """
    solve_name = 'default_solve'

    # Base solve_pattern DataFrame
    base_df = pd.DataFrame({
        'solve_mode': ['single_solve'],
    }, index=pd.Index([solve_name], name='solve_pattern'))

    # Create start_time_durations map from timeline
    if not timeline.index.empty:
        start_time = timeline.index[0]
        duration = timeline.index[-1] - timeline.index[0]
        # Add one timestep to include the last point
        if len(timeline.index) > 1:
            duration += timeline.index[1] - timeline.index[0]
        ...
        warnings.add_warning("Empty timeline - cannot create solve_pattern start_time_durations")
        map_df = pd.DataFrame()
...
def to_cesm(db_path: str) -> Dict[str, pd.DataFrame]:
    ...
    try:
        ...
        cesm['solve_pattern'], cesm['solve_pattern.map.start_time_durations'] = \
            transform_solve_pattern(cesm['timeline'], warnings)
