...
This is only needed for workflows that read from or write to Spine Toolbox databases (IRENA FlexTool).
...
Use the LinkML schema at `model/cesm_v0.1.0.yaml` as the authoritative reference for required and optional fields.
...
----
{
    "type": "path",
    "relative": false,
    "path": "/home/username/sources/flextool/.spinetoolbox/specifications/Tool/flextool3.json"
}
----
...
This must be re-run whenever `model/cesm_v0.1.0.yaml` is modified.
