...
== Cross-sector coupling: electricity and heat

CESM's generic Node/Unit/Link architecture naturally supports multi-energy systems.
This example models a combined heat and power (CHP) plant that produces both electricity and heat, and a heat pump that consumes electricity to produce heat.
...
balance:
  - name: electricity
    flow_scaling_method: use_profile_directly
    flow_profile: [-500, -600, -550]
    penalty_upward: 5000
  - name: heat
    flow_scaling_method: use_profile_directly
    flow_profile: [-200, -300, -250]
    penalty_upward: 5000
...
# CHP plant: gas in, electricity + heat out
node_to_unit:
  - name: natural_gas.chp_plant
    source: natural_gas
    sink: chp_plant

unit_to_node:
  - name: chp_plant.electricity
    source: chp_plant
    sink: electricity
    capacity: 400
  ...
  - name: chp_plant.heat
    ...
    sink: heat
    capacity: 300
    # Thermal efficiency 40% (total efficiency 85%)

# Heat pump: electricity in, heat out
node_to_unit:
  - name: electricity.heat_pump
    source: electricity
    sink: heat_pump
...
unit_to_node:
  - name: heat_pump.heat
    ...
    sink: heat
    capacity: 100
    # COP of 3.0 means efficiency of 300%
----

Key points:

* A CHP plant has one input port (fuel) and two output ports (electricity and heat).
The `conversion_method: constant_efficiency` with the `efficiency` parameter on the unit defines the input-to-output ratio.
Individual port capacities constrain output per carrier.
* A heat pump consumes electricity (input port from the `electricity` balance) and produces heat (output port to the `heat` balance).
A coefficient of performance (COP) of 3.0 is represented as an efficiency of 300%.
* The same architecture generalises to other sector-coupling scenarios: power-to-gas (electrolysis), power-to-hydrogen, combined cooling/heat/power, and so on.
* Material flows (e.g. hydrogen in kg, water in m^3^) can also be modelled using the same structure, with the caveat that the schema's unit annotations are defined in energy terms (MW, MWh) and will not match non-energy commodities.
