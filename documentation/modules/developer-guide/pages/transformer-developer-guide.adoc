...
CESM (stored in DuckDB and defined by link:model/cesm_v0.1.0.yaml[LinkML schema]) serves as the canonical data representation:
...
. Query metadata: `SELECT * FROM "_dataframe_metadata"`
. Query data tables: `SELECT * FROM "unit_ts_availability"`
. Note that index columns are the first N columns based on `index_count`
...
. *Create a processing script* (`scripts/processing/{source}_to_cesm.py`)
...
. *Create a writer* (`src/writers/to_{target}.py`) if the target format
